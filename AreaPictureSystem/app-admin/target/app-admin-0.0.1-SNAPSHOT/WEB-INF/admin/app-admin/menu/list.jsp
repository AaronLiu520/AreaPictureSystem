...
<body>
                    ...
                    <div class="col-lg-6">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>菜单管理目录 <small style="color: red;">除根目录有效菜单只限三级</small></h5>
                    ...
                    <ul>
                      <c:forEach items="${pageList}" var="item" varStatus="status">
                      	...
                      	<!-- jstree-open 打开树形菜单 -->
                      		...
                      		<span onclick="updateFormValue('${item.id}','${item.name}','${item.type}','${item.icon}','${item.url}','${item.orderby}','${item.remark}','${item.pid}')">
                      		<i class="fa ${item.icon}"></i> ${item.name}</span>
                      		
                      			<!-- 二级菜单，根目录 -->
	                      		<c:if test="${item.type == 'HaveMenu'}">
		                      		<ul>
		                      			<c:forEach items="${pageList}" var="subitem" varStatus="status">
		                      				<c:if test="${subitem.pid == item.id}">
		                      					<li data-jstree='{"type":"css"}'>
		                      						<span onclick="updateFormValue('${subitem.id}','${subitem.name}','${subitem.type}','${subitem.icon}','${subitem.url}','${subitem.orderby}','${subitem.remark}','${subitem.pid}')">
		                      							<i class="fa ${subitem.icon}"></i> ${subitem.name}
		                      						</span>
		                      						
		                      						<c:if test="${item.type == 'HaveMenu'}">
		                      							<ul>
		                      								<c:forEach items="${pageList}" var="subitem_end" varStatus="status">
		                      									<c:if test="${subitem_end.pid == subitem.id}">
		                      										<li data-jstree='{"type":"css"}' onclick="updateFormValue('${subitem_end.id}','${subitem_end.name}','${subitem_end.type}','${subitem_end.icon}','${subitem_end.url}','${subitem_end.orderby}','${subitem_end.remark}','${subitem_end.pid}')">
		                      										<span onclick="updateFormValue('${subitem_end.id}','${subitem_end.name}','${subitem_end.type}','${subitem_end.icon}','${subitem_end.url}','${subitem_end.orderby}','${subitem_end.remark}','${subitem_end.pid}')">
		                      											<i class="fa ${subitem_end.icon}"></i> ${subitem_end.name}
		                      										</span>
		                      									</c:if>
		                      								</c:forEach>
		                      							</ul>
                    ...
                    <div class="col-lg-6">
                        <div class="ibox float-e-margins">
                            ...
                            <div class="ibox-content" style="text-align: center;display:none;" id="panel">
                            	<div class="row">
                                	<button id="but_add" onclick="coreate();" type="button" class="btn btn-w-m btn-primary">添加子菜单</button>
                                	<button id="but_update" onclick="update();" type="button" class="btn btn-w-m btn-success">修改菜单</button>
                                </div>
                                	<button id="but_del" onclick="deleteAlert();" type="button" class="btn btn-w-m btn-danger">删除菜单</button>
                            </div>
 ...
 <script type="text/javascript">	
 	...
 	//更新内容传值
	function updateFormValue(id,name,type,icon,url,orderby,remark,pid){
		console.log("加载："+name);
 		//打开操作面板
 		$("#panel").show();
 		//显示面板中的按扭
